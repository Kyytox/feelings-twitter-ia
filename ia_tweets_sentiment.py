...
# # Initialization
# auth = tweepy.OAuthHandler("jedI81cVNadFucrO8awVbzWcp","z8osPI7E3y4PdmMTva6Uwos2grdcl1bYY2GXHH12GV4qQF7F34")
# auth.set_access_token("1524521336420380672-EB8tdVauKSm0k7dYoqMYeaNtzgjeju","BqVdF5VMZInN0s75H0Aelsu0liaIfRJZU1ESzlFmieViQ")
# api = tweepy.API(auth)
...
# TEST 
# test pour trouver le meilleur sentiment d'un tweets 
def nlp_pipeline(text):
...
test = "ne donnons pas les pleins pouvoirs à emmanuel macron et jean-luc mélenchon le bloc national doit lui aussi s’unir et se rassembler notre responsabilité est immense rejoignez l’unionnationale : /0s7c presidentielles2022 elections2022"
text = nlp_pipeline(test)
print(text)
...
# id dernier tweet du compte cerfiaFR 
sauv_id = 1585726779607502849

for i in range(0,40):
  print(i)
  # Retriev tweets by Users, with extended mode and don't take replie tweets and retweets 
  tweets = api.user_timeline(user_id=id_User, max_id=sauv_id, tweet_mode='extended', exclude_replies=True, include_rts=False, count=200)
  # tweets = api.user_timeline(user_id=id_User, tweet_mode='extended', exclude_replies=True, count=1000)

  if tweets:
    if sauv_id == tweets[-1].id:
      break
    ...
    for x in tweets:
      ...
      df = df.append({'Sentiment': l,'Score': st_score, 'Id_Tweet': x.id, 'Negative': scores[0], 'Neutral': scores[1], 'Positive': scores[2],
                      'Text_Tweet': text, 'Date_Tweet': x.created_at}, ignore_index=True)
...
client = tweepy.Client(bearer_token='AAAAAAAAAAAAAAAAAAAAAJbEigEAAAAAU5yBSEbLnSSEmi74TD%2FEW420FS0%3DCUcBqvnO53wT1nznTpA8mDsdCy4N5ihsJQSIqevGRj3LqnjQQo')
...
# BrutFR
id_User = 765849247262633984

# Zemmour
# id_User = 1183418538285027329


# Lest tweet id of the user 
sauv_id = 1587172304278196225

# Retrive maximum tweets of users (max 3200)
tweets = tweepy.Paginator(client.get_users_tweets, id_User, since_id = sauv_id, max_results=100,
...
df = pd.read_csv('Tweets_Macron.csv')
dict_words = {}
...
# insert in CSV
df_words.to_csv('out.csv', index=False, header=True)

# graph Wordcloud
# Les 180 premiers mots les plus utilisés  

df = pd.read_csv('Frequent_Words_Macron_Trier.csv')
...
# txt_sentiments = ["Negatifs","Positifs", "Neutre"]
# colors = ["#800E16", "#003D1E", "#F7B801"]

txt_sentiments = ["Negatifs", "Neutre", "Positifs"]
colors = ["#800E16", "#F7B801", "#003D1E"]
explode = (0.04,0,0)
sub_title = f"{nb_tweet[0]} Tweets analysés"
...
# Title
plt.suptitle('@Macron - Pourcentage des Tweets par sentiment',fontweight='bold', fontsize='18', color='w')
...
# Titles
plt.suptitle('@Macron - Nombre de Tweets par sentiments',fontweight='bold', fontsize='16', color='w')
...
# Titles
plt.suptitle('@Macron - Nombre de Tweets par sentiments regroupé par mois',fontweight='bold', fontsize='18', color='w')
...
# Title
plt.suptitle('@Macron - Nombre d"interactions par sentiments',fontweight='bold', fontsize='18', color='w')
...
for circle in circles:
    ...
    if label == "Interactions aux Tweets Neutres":
      plt.annotate(f"{human_format(nb_inter_neu)} {label}", (x,y+0.26 ) ,va='center', ha='center',fontsize="11", bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', pad=1))
      ...
      plt.annotate(f"{human_format(nb_inter_pos)} {label}", (x,y-0.2) ,va='center', ha='center',fontsize="11", bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', pad=1))
      plt.annotate(f"{nb_tweet_pos[0]} Tweets analysés", (x,y-0.27 ) ,va='center', ha='center', fontsize="11", bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', pad=0.5))
    else:
      plt.annotate(f"{human_format(nb_inter_neg)} {label}", (x,y+0.55 ) ,va='center', ha='center', fontsize="11", bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', pad=1))
      plt.annotate(f"{nb_tweet_neg[0]} Tweets analysés", (x,y+0.475 ) ,va='center', ha='center', fontsize="11", bbox=dict(facecolor='white', edgecolor='black', boxstyle='round', pad=0.5))
...
plt.scatter(nb_tweet_neg[0], nb_inter_neg, s=100,c=colors[0])
plt.scatter(nb_tweet_neu[0], nb_inter_neu, s=100,c=colors[1])
plt.scatter(nb_tweet_pos[0], nb_inter_pos, s=100,c=colors[2])
